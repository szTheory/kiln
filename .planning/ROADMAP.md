...
- [x] **Phase 12: Local Docker / dev environment DX (v0.2.0)** — Optional **`just`/`make`** wrappers (Compose for data plane; Phoenix on host) — **complete 2026-04-22** ([12-01-PLAN.md](phases/12-local-docker-dx/12-01-PLAN.md), [12-01-SUMMARY.md](phases/12-local-docker-dx/12-01-SUMMARY.md))
- [x] **Phase 13: Requirements & roadmap hygiene (v0.2.0)** — Reconcile `REQUIREMENTS.md` checkboxes with shipped Phases 1–9 and `PROJECT.md` **Validated** — **complete 2026-04-22** ([13-01-PLAN.md](phases/13-docs-requirements-reconciliation/13-01-PLAN.md), [13-01-SUMMARY.md](phases/13-docs-requirements-reconciliation/13-01-SUMMARY.md))
...
**Success Criteria** (what must be TRUE):
  1. Starting a run spawns a `Kiln.Agents.SessionSupervisor` with the seven agent processes under per-run `:one_for_one`; killing a single agent process (e.g., Coder) restarts only that role while sibling role processes keep running, and the run does not terminate — the run either recovers or escalates with a typed reason.
...
**Phase artifacts**: `Kiln.WorkUnits` context (current-state table `work_units` + append-only `work_unit_events` + denormalized `blockers_open_count` cache); `Kiln.Agents.SessionSupervisor` (`:one_for_one` over seven role workers, under per-run `RunSubtree` `:one_for_all`) + one module per role (`Planner`, `Coder`, `Tester`, `Reviewer`, `UIUX`, `QAVerifier`, `Mayor`); agent roles implement a common `Kiln.Agents.Role` behaviour; PubSub topic design doc; `~200 LOC` JSONL adapter stub (migration path to `bd` federation if ever needed).
...
**Plans:** 1/1 plans complete
Plans:
- [x] 11-01-PLAN.md — **complete 2026-04-22** ([11-01-SUMMARY.md](phases/11-gameboy-dogfood-vertical-slice/11-01-SUMMARY.md))
...
**Plans:** 1/1 plans complete
Plans:
- [x] 13-01-PLAN.md — complete 2026-04-22 ([13-01-SUMMARY.md](phases/13-docs-requirements-reconciliation/13-01-SUMMARY.md))
...
| 12. Local Docker / dev DX (v0.2.0) | 1/1 | Complete    | 2026-04-22 |
