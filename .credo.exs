...
%{
  ...
  configs: [
    %{
      ...
      # Kiln's custom checks live in `lib/kiln/credo/` and are compiled as part
      # of the project — Credo picks them up via the already-loaded BEAM modules,
      # so no `requires:` entry is needed (adding one caused a module-redefine
      # warning because Credo evaluates the file a second time).
      #
      requires: [],
      #
      ...
      checks: %{
        enabled: [
          #
          ## Kiln custom checks (D-24) — CLAUDE.md anti-pattern enforcement
          #
          {Kiln.Credo.NoProcessPut, []},
          {Kiln.Credo.NoMixEnvAtRuntime, []},

          #
          ## credo_envvar (D-23) — AST-level compile-time-secrets check
          ...
          #  Curated subset — full list in deps/ex_slop/README.md. Any check that
          #  fires on the P1 scaffold can be moved to `disabled:` with a dated
