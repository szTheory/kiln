defmodule KilnWeb.RouterAuthTest do
  ...
  describe "dashboard auth gates" do
    test "unauthenticated dashboard routes redirect to the login page", %{conn: conn} do
      for path <- [
            ~p"/",
            ~p"/templates",
            ~p"/inbox",
    ...
    test "authenticated requests can still render the dashboard", %{conn: conn} do
