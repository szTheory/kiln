defmodule Kiln.Sandboxes.HydratorTest do
  ...
  setup do
    ...
    workspace_dir = Path.join(System.tmp_dir!(), "kiln-hydrator-#{System.unique_integer([:positive])}")
  ...
  test "hydrates artifact refs into the workspace", %{run: run, stage_run: stage_run, workspace_dir: workspace_dir} do
    {:ok, artifact} =
  ...
  test "returns missing_artifact when the CAS blob cannot be resolved", %{workspace_dir: workspace_dir} do
